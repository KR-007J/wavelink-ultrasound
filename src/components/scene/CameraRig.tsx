...
export const CameraRig: React.FC = () => {
  ...
  useFrame((state, delta) => {
    ...
    // 2. Gimbal Micro-Parallax Drift (Continuous Figure-8 Lissajous subtle motion)
    ...
    const parallaxY = -state.pointer.y * 0.09;

    // 4. Impact Shake Decay (150ms decay)
    ...
    camera.position.x = THREE.MathUtils.lerp(
      camera.position.x,
      targetPos.current.x + driftX + parallaxX + shakeOffsetX,
      0.06
    );
    ...
    camera.position.z = THREE.MathUtils.lerp(camera.position.z, targetPos.current.z, 0.06);
